...
- 作用范围：**选区**、**全文**、**当前查找命中**（先查找再批量变换命中文本）。
- **JSON 工具箱**：美化、压缩、键排序、校验、扁平化、路径取值、数组 ↔ CSV、结构对比（`---` 分隔）、字符串转义。
- **常用配方**：清理粘贴、SQL 整理、JSON 系列、配置发布前、日志缩水、编码救火。
- 清洗 / 行变换 / SQL·XML·HTML / URL·Base64·Unicode / CSV / 文本统计。
- 预览后再应用；统计与 JSON 校验/结构对比仅预览不写回。
- 安全护栏：
  - 选区为空时应用会确认「将改全文」；
  - 破坏性工具（去重、排序、删空行等）应用前确认；
  - 查找命中模式仅允许逐段安全变换（大小写、编解码、JSON 转义等），配方/结构对比/统计等会禁用；
  - 命中批量优先用编辑器实时匹配，降低路径对齐误差。

### 格式化

- **JSON**、**SQL**（独立 worker）、**XML/HTML**（启发式缩进，复杂文档请复核）。
- JSON / XML / HTML 支持压缩。
- 其他语言尽量走 Monaco 内置格式化（视语言支持情况而定）。
